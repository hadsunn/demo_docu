...
More information about the lock modes and locking strategies can be found in [Section 13.3](</II. The SQL Language/13. Concurrency Control/explicit-locking.md>).
...
The lock mode specifies which locks this lock conflicts with. Lock modes are described in [Section 13.3](</II. The SQL Language/13. Concurrency Control/explicit-locking.md>).
...
There is no `LOCK TABLE` in the SQL standard, which instead uses `SET TRANSACTION` to specify concurrency levels on transactions. PostgreSQL supports that too; see [SET TRANSACTION](</VI. Reference/I. SQL Commands/sql-set-transaction.md>) for details.
