...
The data type(s) of the procedure's arguments (optionally schema-qualified), if any. See [DROP PROCEDURE](</VI. Reference/I. SQL Commands/sql-dropprocedure.md>) for the details of how the procedure is looked up using the argument data type(s).
...
Change whether the procedure is a security definer or not. The key word `EXTERNAL` is ignored for SQL conformance. See [CREATE PROCEDURE](</VI. Reference/I. SQL Commands/sql-createprocedure.md>) for more information about this capability.
