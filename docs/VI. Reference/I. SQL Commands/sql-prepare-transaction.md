...
Once prepared, a transaction can later be committed or rolled back with [`COMMIT PREPARED`](</VI. Reference/I. SQL Commands/sql-commit-prepared.md>), respectively. Those commands can be issued from any session, not only the one that executed the original transaction.
...
This command must be used inside a transaction block. Use [`BEGIN`](</VI. Reference/I. SQL Commands/sql-begin.md>) to start one.
...
[COMMIT PREPARED](</VI. Reference/I. SQL Commands/sql-commit-prepared.md>)
