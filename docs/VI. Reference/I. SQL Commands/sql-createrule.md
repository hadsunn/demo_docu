...
A view that is simple enough to be automatically updatable (see [CREATE VIEW](</VI. Reference/I. SQL Commands/sql-createview.md>)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.

Another alternative worth considering is to use `INSTEAD OF` triggers (see [CREATE TRIGGER](</VI. Reference/I. SQL Commands/sql-createtrigger.md>)) in place of rules.
...
[ALTER RULE](</VI. Reference/I. SQL Commands/sql-alterrule.md>)
