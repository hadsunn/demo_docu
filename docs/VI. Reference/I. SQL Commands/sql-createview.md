...
This should be used if the view is intended to provide row-level security. See [Section 41.5](</V. Server Programming/41. The Rule System/rules-privileges.md>) for full details.
...
All of the above options can be changed on existing views using [`ALTER VIEW`](</VI. Reference/I. SQL Commands/sql-alterview.md>).

_`query`_
    

A [`SELECT`](</VI. Reference/I. SQL Commands/sql-select.md>) command which will provide the columns and rows of the view.
...
Use the [`DROP VIEW`](</VI. Reference/I. SQL Commands/sql-dropview.md>) statement to drop views.
