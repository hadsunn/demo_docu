...
Discards all cached sequence-related state, including `currval()`/`lastval()` information and any preallocated sequence values that have not yet been returned by `nextval()`. (See [CREATE SEQUENCE](</VI. Reference/I. SQL Commands/sql-createsequence.md>) for a description of preallocated sequence values.)
