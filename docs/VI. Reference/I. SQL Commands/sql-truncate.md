...
`TRUNCATE` is not MVCC-safe. After truncation, the table will appear empty to concurrent transactions, if they are using a snapshot taken before the truncation occurred. See [Section 13.6](</II. The SQL Language/13. Concurrency Control/mvcc-caveats.md>) for more details.
...
`TRUNCATE` can be used for foreign tables if supported by the foreign data wrapper, for instance, see [postgres_fdw](</VIII. Appendixes/F. Additional Supplied Modules and Extensions/postgres-fdw.md>).
...
[DELETE](</VI. Reference/I. SQL Commands/sql-delete.md>)
