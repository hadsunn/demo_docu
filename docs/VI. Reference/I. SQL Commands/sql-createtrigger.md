...
When the `CONSTRAINT` option is specified, this command creates a _constraint trigger_. This is the same as a regular trigger except that the timing of the trigger firing can be adjusted using [`SET CONSTRAINTS`](</VI. Reference/I. SQL Commands/sql-set-constraints.md>). Constraint triggers must be `AFTER ROW` triggers on plain tables (not foreign tables). They can be fired either at the end of the statement causing the triggering event, or at the end of the containing transaction; in the latter case they are said to be _deferred_. A pending deferred-trigger firing can also be forced to happen immediately by using `SET CONSTRAINTS`. Constraint triggers are expected to raise an exception when the constraints they implement are violated.
...
Refer to [Chapter 39](</V. Server Programming/39. Triggers/39. Triggers.md>) for more information about triggers.
...
The default timing of the trigger. See the [CREATE TABLE](</VI. Reference/I. SQL Commands/sql-createtable.md>) documentation for details of these constraint options. This can only be specified for constraint triggers.
...
Use [`DROP TRIGGER`](</VI. Reference/I. SQL Commands/sql-droptrigger.md>) to remove a trigger.
...
There are a few built-in trigger functions that can be used to solve common problems without having to write your own trigger code; see [Section 9.28](</II. The SQL Language/9. Functions and Operators/functions-trigger.md>).
...
[Section 39.4](</V. Server Programming/39. Triggers/trigger-example.md>) contains a complete example of a trigger function written in C.
...
[ALTER TRIGGER](</VI. Reference/I. SQL Commands/sql-altertrigger.md>)
