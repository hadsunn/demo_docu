...
The new user will have the `REPLICATION` privilege, which is described more fully in the documentation for [CREATE ROLE](</VI. Reference/I. SQL Commands/sql-createrole.md>).

`--no-replication`
    

The new user will not have the `REPLICATION` privilege, which is described more fully in the documentation for [CREATE ROLE](</VI. Reference/I. SQL Commands/sql-createrole.md>). This is the default.
...
In case of difficulty, see [CREATE ROLE](</VI. Reference/I. SQL Commands/sql-createrole.md>) for discussions of potential problems and error messages. The database server must be running at the targeted host. Also, any default connection settings and environment variables used by the libpq front-end library will apply.
...
[dropuser](</VI. Reference/II. PostgreSQL Client Applications/app-dropuser.md>), [createrole_self_grant](runtime-config-client.html#GUC-CREATEROLE-SELF-GRANT)
