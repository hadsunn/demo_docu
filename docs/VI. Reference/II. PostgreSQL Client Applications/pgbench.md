...
The default is simple query protocol. (See [Chapter 55](</VII. Internals/55. FrontendBackend Protocol/55. FrontendBackend Protocol.md>) for more information.)
...
These commands delimit the start and end of a pipeline of SQL statements. In pipeline mode, statements are sent to the server without waiting for the results of previous statements. See [Section 34.5](</IV. Client Interfaces/34. libpq — C Library/libpq-pipeline-mode.md>) for more details. Pipeline mode requires the use of extended query protocol.
...
You may specify the [Table Access Method](</VII. Internals/tableam.md>)). For example, a hypothetical default Table Access Method for the tables that pgbench creates called `wuzza` can be specified with:
