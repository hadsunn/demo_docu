...
Each TOAST-able data type specifies a default strategy for columns of that data type, but the strategy for a given table column can be altered with [`ALTER TABLE ... SET STORAGE`](</VI. Reference/I. SQL Commands/sql-altertable.md>).

`TOAST_TUPLE_TARGET` can be adjusted for each table using [`ALTER TABLE ... SET (<toast_tuple_target>)
