...
See also [`pg_shdepend`](</VII. Internals/53. System Catalogs/catalog-pg-shdepend.md>), which performs a similar function for dependencies involving objects that are shared across a database cluster.
...
`refclassid` `oid` (references [`pg_class`](</VII. Internals/53. System Catalogs/catalog-pg-class.md>).`oid`) The OID of the system catalog the referenced object is in  
