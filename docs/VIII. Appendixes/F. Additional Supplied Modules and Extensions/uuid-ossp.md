...
[F.49.1. `uuid-ossp` Functions](</VIII. Appendixes/F. Additional Supplied Modules and Extensions/uuid-ossp.md#f491-uuid-ossp-functions>)


[F.49.2. Building `uuid-ossp`](</VIII. Appendixes/F. Additional Supplied Modules and Extensions/uuid-ossp.md#f492-building-uuid-ossp>)


[F.49.3. Author](</VIII. Appendixes/F. Additional Supplied Modules and Extensions/uuid-ossp.md#f493-author>)



The `uuid-ossp` module provides functions to generate universally unique identifiers (UUIDs) using one of several standard algorithms. There are also functions to produce certain special UUID constants. This module is only necessary for special requirements beyond what is available in core PostgreSQL. See [Section 9.14](</II. The SQL Language/9. Functions and Operators/functions-uuid.md>) for built-in ways to generate UUIDs.
...
`uuid_generate_v3` ( _`namespace`_ `uuid`, _`name`_ `text` ) → `uuid` Generates a version 3 UUID in the given namespace using the specified input name. The namespace should be one of the special constants produced by the `uuid_ns_*()` functions shown in [Table F.35](uuid-ossp.html#UUID-OSSP-CONSTANTS "Table F.35. Functions Returning UUID Constants"). (It could be any UUID in theory.) The name is an identifier in the selected namespace. For example:
