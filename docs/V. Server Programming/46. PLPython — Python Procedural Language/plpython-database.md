...
Query parameters and result row fields are converted between PostgreSQL and Python data types as described in [Section 46.2](</V. Server Programming/46. PLPython — Python Procedural Language/plpython-data.md>).

When you prepare a plan using the PL/Python module it is automatically saved. Read the SPI documentation ([Chapter 47](</V. Server Programming/47. Server Programming Interface/47. Server Programming Interface.md>)). For example:
