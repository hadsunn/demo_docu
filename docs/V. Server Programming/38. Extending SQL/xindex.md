...
B-trees require a comparison support function, and allow four additional support functions to be supplied at the operator class author's option, as shown in [Table 38.9](<xindex#XINDEX-BTREE-SUPPORT-TABLE>).
...
GiST indexes have eleven support functions, six of which are optional, as shown in [Table 38.11](<xindex#XINDEX-GIST-SUPPORT-TABLE>).)
...
SP-GiST indexes have six support functions, one of which is optional, as shown in [Table 38.12](<xindex#XINDEX-SPGIST-SUPPORT-TABLE>).)
...
GIN indexes have seven support functions, four of which are optional, as shown in [Table 38.13](<xindex#XINDEX-GIN-SUPPORT-TABLE>).)
...
BRIN indexes have five basic support functions, one of which is optional, as shown in [Table 38.14](<xindex#XINDEX-BRIN-SUPPORT-TABLE>).)
...
Another SQL feature that requires even more data-type-specific knowledge is the `RANGE` _`offset`_ `PRECEDING`/`FOLLOWING` framing option for window functions (see [Section 4.2.8](sql-expressions.html#SYNTAX-WINDOW-FUNCTIONS "4.2.8. Window Function Calls")). For a query such as
