...
`TG_RELID` `oid` (references [`pg_class`](</VII. Internals/53. System Catalogs/catalog-pg-class.md>).`oid`) #
...
PL/pgSQL can be used to define [event triggers](</V. Server Programming/40. Event Triggers/40. Event Triggers.md>). PostgreSQL requires that a function that is to be called as an event trigger must be declared as a function with no arguments and a return type of `event_trigger`.
