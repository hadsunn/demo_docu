...
<p>643.72410666.00067-07 97 01-ЛУ</p></th>
...
<p><strong>Руководство по безопасности</strong></p></td>
</tr>
<tr>
<td colspan="2" style="text-align: center;"><strong>643.72410666.00067-07 97 01-27</strong></td>
</tr>
<tr>
...
<tr>
<td colspan="2" style="text-align: center;">Листов 130</td>
</tr>
<tr>
<td style="text-align: center;"></td>
<td style="text-align: center;"></td>
...
<th><p>Все примеры в данном документе приведены для СУБД «Jatoba» версии ядра .x, 6 для других версий все шаги выполняются аналогично, разница состоит в именах директорий.</p>
...
[1. Этапы развертывания СУБД на хостах [6](#этапы-развертывания-субд-на-хостах)](#этапы-развертывания-субд-на-хостах)

[1.1. Первый этап. Топология хостов СУБД [6](#первый-этап.-топология-хостов-субд)](#первый-этап.-топология-хостов-субд)

[1.2. Второй этап. Установка СУБД на хостах [7](#второй-этап.-установка-субд-на-хостах)](#второй-этап.-установка-субд-на-хостах)

[1.3. Третий этап. Установка JDS [7](#третий-этап.-установка-jds)](#третий-этап.-установка-jds)

[1.4. Четвертый этап. Подготовка хостов. Настройка SSH соединений [7](#четвертый-этап.-подготовка-хостов.-настройка-ssh-соединений)](#четвертый-этап.-подготовка-хостов.-настройка-ssh-соединений)

[1.5. Пятый этап. Подготовка SSL подключений [7](#подготовку-хостов-для-целевых-субднастройку-ssh-соединений.пятый-этап.-подготовка-ssl-подключений)](#подготовку-хостов-для-целевых-субднастройку-ssh-соединений.пятый-этап.-подготовка-ssl-подключений)

[1.6. Шестой этап. Установка компонентов СУБД [7](#шестой-этап.-установка-компонентов-субд)](#шестой-этап.-установка-компонентов-субд)

[2. Настройка SSH-соединений JDS [8](#настройка-ssh-соединений-jds)](#настройка-ssh-соединений-jds)

[2.1. Создание пользователя ОС jdscontrol на целевом хосте [8](#создание-пользователя-ос-jdscontrol-на-целевом-хосте)](#создание-пользователя-ос-jdscontrol-на-целевом-хосте)

[2.2. Копирование скрипта на целевой хост [9](#копирование-скрипта-на-целевой-хост)](#копирование-скрипта-на-целевой-хост)

[2.3. Выполнение скрипта на целевом хосте [10](#выполнение-скрипта-на-целевом-хосте)](#выполнение-скрипта-на-целевом-хосте)

[2.4. Создание домашнего каталога пользователя JDS на служебном хосте [11](#создание-домашнего-каталога-пользователя-jds-на-служебном-хосте)](#создание-домашнего-каталога-пользователя-jds-на-служебном-хосте)

[2.5. Создание ключа SSH для пользователя JDS на служебном хосте [11](#создание-ключа-ssh-для-пользователя-jds-на-служебном-хосте)](#создание-ключа-ssh-для-пользователя-jds-на-служебном-хосте)

[2.6. Передача ключа SSH на целевую СУБД [12](#передача-ключа-ssh-на-целевую-субд)](#передача-ключа-ssh-на-целевую-субд)

[2.7. Подключение JDS к целевой СУБД (SSH и PASSWORD) [13](#подключение-jds-к-целевой-субд-ssh-и-password)](#подключение-jds-к-целевой-субд-ssh-и-password)

[2.7.1. Хост [14](#группахост-субд.хост)](#группахост-субд.хост)

[2.7.2. Настройка конфигурационного файла pg_hba.conf [15](#настройка-конфигурационного-файла-pg_hba.conf)](#настройка-конфигурационного-файла-pg_hba.conf)

[2.7.3. СУБД [15](#субд)](#субд)

[3. Настройка SSL-соединений [17](#настройка-ssl-соединений)](#настройка-ssl-соединений)

[3.1. Создание и конвертация сертификатов [19](#создание-и-конвертация-сертификатов)](#создание-и-конвертация-сертификатов)

[3.2. Пути хранения сертификатов и ключей компонентов СУБД [22](#пути-хранения-сертификатов-и-ключей-компонентов-субд)](#пути-хранения-сертификатов-и-ключей-компонентов-субд)

[4. Подключение JDS к целевой СУБД по SSL [26](#подключение-jds-к-целевой-субд-по-ssl)](#подключение-jds-к-целевой-субд-по-ssl)

[5. Подключение JDS к служебной СУБД по SSL [28](#подключение-jds-к-служебной-субд-по-ssl)](#подключение-jds-к-служебной-субд-по-ssl)

[6. Подключение к Web-интерфейсу JDS по SSL [30](#подключение-к-web-интерфейсу-jds-по-ssl)](#подключение-к-web-интерфейсу-jds-по-ssl)

[7. Настройка SSL компонента ja_Dog [31](#настройка-ssl-компонента-ja_dog)](#настройка-ssl-компонента-ja_dog)

[7.1. Настройка SSL между узлами кластера [31](#настройка-ssl-между-узлами-кластера)](#настройка-ssl-между-узлами-кластера)

[7.2. Настройка доступа ja_Dog к СУБД по SSL [33](#настройка-доступа-ja_dog-к-субд-по-ssl)](#настройка-доступа-ja_dog-к-субд-по-ssl)

[7.3. Подключение кластера «ja_Dog» к JDS разделе «Ландшафт» [34](#подключение-кластера-ja_dog-к-jds-разделе-ландшафт)](#подключение-кластера-ja_dog-к-jds-разделе-ландшафт)

[7.3.1. Настройка Rest API [35](#настройка-rest-api)](#настройка-rest-api)

[7.3.2. Проверочные мероприятия [37](#проверочные-мероприятия)](#проверочные-мероприятия)

[7.3.3. Добавление существующего кластера ja_Dog [39](#добавление-существующего-кластера-ja_dog)](#добавление-существующего-кластера-ja_dog)

[8. Раздел JDS «Анализ запросов» [41](#раздел-jds-анализ-запросов)](#раздел-jds-анализ-запросов)

[8.1. Настройка на узле, отдельном от JDS [41](#настройка-на-узле-отдельном-от-jds)](#настройка-на-узле-отдельном-от-jds)

[8.1.1. Подключение Explain в JDS [42](#подключение-explain-в-jds)](#подключение-explain-в-jds)

[8.2. Настройка на одном узле с JDS [43](#настройка-на-одном-узле-с-jds)](#настройка-на-одном-узле-с-jds)

[9. Раздел JDS «Мониторинг» [45](#раздел-jds-мониторинг)](#раздел-jds-мониторинг)

[9.1. Система «Prometheus» [45](#система-prometheus)](#система-prometheus)

[9.2. Экспортер «jatoba\*\_node_exporter» [47](#экспортер-jatoba_node_exporter)](#экспортер-jatoba_node_exporter)

[9.3. Экспортер «jatoba\*\_postgres_exporter» [48](#экспортер-jatoba_postgres_exporter)](#экспортер-jatoba_postgres_exporter)

[9.4. Экспортера «jatoba\*\_sql_exporter» [50](#экспортера-jatoba_sql_exporter)](#экспортера-jatoba_sql_exporter)

[9.5. Настройка подключения в JDS [52](#настройка-подключения-в-jds)](#настройка-подключения-в-jds)

[9.6. Настройка «Grafana» [52](#настройка-grafana)](#настройка-grafana)

[10. ja_log [54](#ja_log)](#ja_log)

[10.1. ja_log (сервер) [54](#ja_log-сервер)](#ja_log-сервер)

[10.2. ja_log (агент) [55](#ja_log-агент)](#ja_log-агент)

[10.3. JDS. Подключение ja_Log для сбора событий [57](#jds.-подключение-ja_log-для-сбора-событий)](#jds.-подключение-ja_log-для-сбора-событий)

[11. jaPooler [59](#japooler)](#japooler)

[11.1. jaPooler. Подключение по SSL [59](#japooler.-подключение-по-ssl)](#japooler.-подключение-по-ssl)

[11.2. jaPooler. Подключение по SSL с паролем [62](#japooler.-подключение-по-ssl-с-паролем)](#japooler.-подключение-по-ssl-с-паролем)

[12. ja_sync_ldap [67](#ja_sync_ldap)](#ja_sync_ldap)

[13. Пример настройки SSL-соединений JDS [70](#пример-настройки-ssl-соединений-jds)](#пример-настройки-ssl-соединений-jds)

[13.1. Требуемое программное обеспечение [70](#требуемое-программное-обеспечение)](#требуемое-программное-обеспечение)

[13.2. Пользователи [70](#пользователи)](#пользователи)

[13.3. Каталог хранения сертификатов [70](#каталог-хранения-сертификатов)](#каталог-хранения-сертификатов)

[13.4. Создание конфигурационных файлов OpenSSL [70](#создание-конфигурационных-файлов-openssl)](#создание-конфигурационных-файлов-openssl)

[13.4.1. Конфигурационный файл OpenSSL корневого ЦС [70](#конфигурационный-файл-openssl-корневого-цс)](#конфигурационный-файл-openssl-корневого-цс)

[13.4.2. Конфигурационный файл OpenSSL промежуточного ЦС [71](#конфигурационный-файл-openssl-промежуточного-цс)](#конфигурационный-файл-openssl-промежуточного-цс)

[13.5. Создание самоподписанных сертификатов [72](#создание-самоподписанных-сертификатов)](#создание-самоподписанных-сертификатов)

[13.5.1. Самоподписанный сертификат корневого ЦС (Root CA) [72](#самоподписанный-сертификат-корневого-цс-root-ca)](#самоподписанный-сертификат-корневого-цс-root-ca)

[13.5.2. Самоподписанный сертификат промежуточного ЦС (Root CA) [73](#самоподписанный-сертификат-промежуточного-цс-root-ca)](#самоподписанный-сертификат-промежуточного-цс-root-ca)

[13.5.3. Самоподписанный сертификат сервера СУБД (Root CA) [74](#самоподписанный-сертификат-сервера-субд-root-ca)](#самоподписанный-сертификат-сервера-субд-root-ca)

[13.6. Создание клиентских сертификатов [80](#создание-клиентских-сертификатов)](#создание-клиентских-сертификатов)

[13.6.1. Клиентский сертификат пользователя postgres [80](#клиентский-сертификат-пользователя-postgres)](#клиентский-сертификат-пользователя-postgres)

[13.6.2. Клиентский сертификат пользователя JDS [83](#клиентский-сертификат-пользователя-jds)](#клиентский-сертификат-пользователя-jds)

[13.6.3. Клиентский сертификат пользователя test_user [86](#клиентский-сертификат-пользователя-test_user)](#клиентский-сертификат-пользователя-test_user)

[13.7. Структура хранения сертификатов [89](#структура-хранения-сертификатов)](#структура-хранения-сертификатов)

[13.8. Настройка СУБД для SSL-соединения [89](#настройка-субд-для-ssl-соединения)](#настройка-субд-для-ssl-соединения)

[13.9. Создание цели (Target) с SSL-соединением [91](#создание-цели-target-с-ssl-соединением)](#создание-цели-target-с-ssl-соединением)

[13.10. Настройка компонента JDS для SSL-соединений [92](#в-поле-функциональность-выбрать-все-функциональности.нажать-ок.настройка-компонента-jds-для-ssl-соединений)](#в-поле-функциональность-выбрать-все-функциональности.нажать-ок.настройка-компонента-jds-для-ssl-соединений)

[14. Настройки СУБД и ее компонент по умолчанию, которые могут быть использованы для НСД [96](#настройки-субд-и-ее-компонент-по-умолчанию-которые-могут-быть-использованы-для-нсд)](#настройки-субд-и-ее-компонент-по-умолчанию-которые-могут-быть-использованы-для-нсд)

[14.1. Компоненты хранящие пароли (ключи) [97](#компоненты-хранящие-пароли-ключи)](#компоненты-хранящие-пароли-ключи)

[14.2. Перечень настроек авторизации и аутентификации СУБД «Jatoba» и входящих в ее состав компонент, отвечающих за ИБ [100](#перечень-настроек-авторизации-и-аутентификации-субд-jatoba-и-входящих-в-ее-состав-компонент-отвечающих-за-иб)](#перечень-настроек-авторизации-и-аутентификации-субд-jatoba-и-входящих-в-ее-состав-компонент-отвечающих-за-иб)

[15. Реагирование на инциденты ИБ [116](#реагирование-на-инциденты-иб)](#реагирование-на-инциденты-иб)

[15.1. Нарушение целостности и последующая блокировка пользователей СУБД [117](#нарушение-целостности-и-последующая-блокировка-пользователей-субд)](#нарушение-целостности-и-последующая-блокировка-пользователей-субд)

[15.2. Превышение попыток количества неудачных попыток входа в СУБД [123](#превышение-попыток-количества-неудачных-попыток-входа-в-субд)](#превышение-попыток-количества-неудачных-попыток-входа-в-субд)

[Приложение 1 [127](#_Toc195610205)](#_Toc195610205)

[Установка службы JDS.PasDoctor [127](#_Toc195610206)](#_Toc195610206)

[Перечень сокращений [129](#_Toc195610207)](#_Toc195610207)
...
![](../docs/assets/images/cert6.8.4,5.12.4,4.17.4/sec_guide/media/image3.emf)
...
ручную установку;автоматизированную установку.Ручная установка описана в документе «Руководство по установке».
...
![](../docs/assets/images/cert6.8.4,5.12.4,4.17.4/sec_guide/media/image22.emf)
