...
The _`evtId`_ parameter indicates which `PGEVT` event occurred. The _`evtInfo`_ pointer must be cast to the appropriate structure type to obtain further information about the event. The _`passThrough`_ parameter is the pointer provided to [`PQregisterEventProc`](libpq-events.html#LIBPQ-PQREGISTEREVENTPROC) when the event procedure was registered. The function should return a non-zero value if it succeeds and zero if it fails.
