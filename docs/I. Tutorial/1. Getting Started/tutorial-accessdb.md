---
title: 1.4. Accessing a Database
---
...
Once you have created a database, you can access it by:
  ...
  * Writing a custom application, using one of the several available language bindings. These possibilities are discussed further in [Part IV](</IV. Client Interfaces/IV. Client Interfaces.md>).
...
and `psql` will quit and return you to your command shell. (For more internal commands, type `\?` at the `psql` prompt.) The full capabilities of `psql` are documented in [psql](</VI. Reference/II. PostgreSQL Client Applications/app-psql.md>). In this tutorial we will not use these features explicitly, but you can use them yourself when it is helpful.
