...
or otherwise a note about which tests failed. See [Section 33.2](</III. Server Administration/33. Regression Tests/regress-evaluation.md>) below before assuming that a “failure” represents a serious problem.
...
To run the tests after installation (see [Chapter 17](</III. Server Administration/17. Installation from Source Code/17. Installation from Source Code.md>), then type:
...
Some of these auxiliary test suites use the TAP infrastructure explained in [Section 33.4](</III. Server Administration/33. Regression Tests/regress-tap.md>). The TAP-based tests are run only when PostgreSQL was configured with the option `--enable-tap-tests`. This is recommended for development, but can be omitted if there is no suitable Perl installation.
...
This can be useful to enable additional logging, adjust resource limits, or enable extra run-time checks such as [debug_discard_caches](runtime-config-developer.html#GUC-DEBUG-DISCARD-CACHES).
