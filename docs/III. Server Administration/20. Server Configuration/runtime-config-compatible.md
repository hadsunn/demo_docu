---
title: 20.13. Version and Platform Compatibility
---
...
When the database generates SQL, force all identifiers to be quoted, even if they are not (currently) keywords. This will affect the output of `EXPLAIN` as well as the results of functions like `pg_get_viewdef`. See also the `--quote-all-identifiers` option of [pg_dump](</VI. Reference/II. PostgreSQL Client Applications/app-pgdump.md>).
...
Refer to [Section 9.2](</II. The SQL Language/9. Functions and Operators/functions-comparison.md>) for related information.
