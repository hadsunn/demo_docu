---
title: 19.2. Creating a Database Cluster
---
...
Alternatively, you can run `initdb` via the [pg_ctl](</VI. Reference/III. PostgreSQL Server Applications/app-pg-ctl.md>) program like so:
    
    
    $ **pg_ctl -D /usr/local/pgsql/data initdb**
    

This may be more intuitive if you are using `pg_ctl` for starting and stopping the server (see [Section 19.3](</III. Server Administration/19. Server Setup and Operation/server-start.md>)), so that `pg_ctl` would be the sole command you use for managing the database server instance.
