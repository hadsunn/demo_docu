---
title: 24.3. Character Set Support
---
...
To enable automatic character set conversion, you have to tell PostgreSQL the character set (encoding) you would like to use in the client. There are several ways to accomplish this:
  ...
  * libpq ([Section 34.11](</IV. Client Interfaces/34. libpq — C Library/libpq-control.md>)) has functions to control the client encoding.
...
PostgreSQL allows conversion between any two character sets for which a conversion function is listed in the [`pg_conversion`](</VII. Internals/53. System Catalogs/catalog-pg-conversion.md>). (To be used for automatic client/server conversions, a conversion must be marked as “default” for its character set pair.)
