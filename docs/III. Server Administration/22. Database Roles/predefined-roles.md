...
pg_checkpoint | Allow executing the [`CHECKPOINT`](</VI. Reference/I. SQL Commands/sql-checkpoint.md>) command.  
...
pg_create_subscription | Allow users with `CREATE` permission on the database to issue [`CREATE SUBSCRIPTION`](</VI. Reference/I. SQL Commands/sql-createsubscription.md>).  
...
Administrators can grant access to these roles to users using the [`GRANT`](</VI. Reference/I. SQL Commands/sql-grant.md>) command, for example:
