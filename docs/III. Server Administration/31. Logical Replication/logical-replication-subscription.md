...
The subscription is added using [`CREATE SUBSCRIPTION`](</VI. Reference/I. SQL Commands/sql-createsubscription.md>).
...
Create publications for the tables. The publications `pub2` and `pub3a` disallow some [`publish`](<sql-createpublication#SQL-CREATEPUBLICATION-WITH-PUBLISH)>)).
