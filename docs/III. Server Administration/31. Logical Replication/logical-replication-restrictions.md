...
Logical replication currently has the following restrictions or missing functionality. These might be addressed in future releases.
  ...
  * Large objects (see [Chapter 35](</IV. Client Interfaces/35. Large Objects/35. Large Objects.md>)) are not replicated. There is no workaround for that, other than storing data in normal tables.
