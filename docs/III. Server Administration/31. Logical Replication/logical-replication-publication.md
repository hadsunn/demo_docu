...
A publication is created using the [`CREATE PUBLICATION`](</VI. Reference/I. SQL Commands/sql-createpublication.md>) command and may later be altered or dropped using corresponding commands.

The individual tables can be added and removed dynamically using [`ALTER PUBLICATION`](</VI. Reference/I. SQL Commands/sql-alterpublication.md>). Both the `ADD TABLE` and `DROP TABLE` operations are transactional; so the table will start or stop replicating at the correct snapshot once the transaction has committed.
