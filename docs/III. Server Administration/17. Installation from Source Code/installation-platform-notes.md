...
This section documents additional platform-specific issues regarding the installation and setup of PostgreSQL. Be sure to read the installation instructions, and in particular [Section 17.1](</III. Server Administration/17. Installation from Source Code/install-requirements.md>) regarding the interpretation of regression test results.
...
PostgreSQL can be built using Cygwin, a Linux-like environment for Windows, but that method is inferior to the native Windows build (see [Chapter 18](</III. Server Administration/18. Installation from Source Code on Windows/18. Installation from Source Code on Windows.md>)) and running a server under Cygwin is no longer recommended.
...
Yes, using DTrace is possible. See [Section 28.5](</III. Server Administration/28. Monitoring Database Activity/dynamic-trace.md>) for further information.
