...
In addition, you can define your own range types; see [CREATE TYPE](</VI. Reference/I. SQL Commands/sql-createtype.md>) for more information.
...
See [CREATE TYPE](</VI. Reference/I. SQL Commands/sql-createtype.md>) for more information about creating range types.
...
You can use the [`btree_gist`](</VIII. Appendixes/F. Additional Supplied Modules and Extensions/btree-gist.md>) extension to define exclusion constraints on plain scalar data types, which can then be combined with range exclusions for maximum flexibility. For example, after `btree_gist` is installed, the following constraint will reject overlapping ranges only if the meeting room numbers are equal:
