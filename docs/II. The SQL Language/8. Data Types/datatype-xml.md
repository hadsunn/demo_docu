...
The `xml` data type can be used to store XML data. Its advantage over storing XML data in a `text` field is that it checks the input values for well-formedness, and there are support functions to perform type-safe operations on it; see [Section 9.15](</II. The SQL Language/9. Functions and Operators/functions-xml.md>). Use of this data type requires the installation to have been built with `configure --with-libxml`.
...
Limits and compatibility notes for the `xml` data type can be found in [Section D.3](</VIII. Appendixes/D. SQL Conformance/xml-limits-conformance.md>).
