...
Acquired by `VACUUM` (without `FULL`), `ANALYZE`, `CREATE INDEX CONCURRENTLY`, `CREATE STATISTICS`, `COMMENT ON`, `REINDEX CONCURRENTLY`, and certain [`ALTER INDEX`](</VI. Reference/I. SQL Commands/sql-alterindex.md>) variants (for full details see the documentation of these commands).
...
Acquired by `CREATE TRIGGER` and some forms of [`ALTER TABLE`](</VI. Reference/I. SQL Commands/sql-altertable.md>).
...
Like all locks in PostgreSQL, a complete list of advisory locks currently held by any session can be found in the [`pg_locks`](</VII. Internals/54. System Views/view-pg-locks.md>) system view.
