...
[enable_parallel_append](runtime-config-query.html#GUC-ENABLE-PARALLEL-APPEND) can be used to disable this feature.

### 15.3.5. Parallel Plan Tips #

If a query that is expected to do so does not produce a parallel plan, you can try reducing [parallel_setup_cost](<runtime-config-query#GUC-PARALLEL-SETUP-COST)>) for information on why this may be the case.
