...
To force the planner to follow the join order laid out by explicit `JOIN`s, set the [join_collapse_limit](runtime-config-query.html#GUC-JOIN-COLLAPSE-LIMIT) run-time parameter to 1. (Other possible values are discussed below.)
